"""
Run image_denoising on all images in data/{high_5, mid_5, low_5} for all 11
INR methods, then compute per-group averages of best/final PSNR & SSIM.

Mirrors run_fitting_groups.py / run_inpaint_groups.py / run_superresolution_groups.py.
The denoise task internally builds a noisy observation (Gaussian or
Poisson-Gaussian per training.noise_type), trains on the noisy image, and
evaluates PSNR/SSIM against the CLEAN ground truth.

Usage:
    python benchmark/run_denoise_groups.py
    python benchmark/run_denoise_groups.py --groups high_5 --methods siren
    python benchmark/run_denoise_groups.py --noise_tau 40 --noise_readout_snr 1.0

Outputs:
    results/image_denoise_groups/
        summary.json          # full per-image + averaged numbers
        summary.md            # readable markdown table
        <group>/<method>/
            <name>_results.pt
            <name>/<name>_ep*.png, <name>_final.png,
                   <name>_noisy.png, <name>_clean.png
"""
...
def parse_args():
    ...
    p.add_argument('--config_dir', default='configs/experiments/image_denoising_3mean',
                   help='Per-method YAML configs (one <method>.yaml each). Falls back to '
                        'image_fitting_3mean if this dir is missing.')
    p.add_argument('--out_root',   default='results/image_denoise_groups')
    ...
    p.add_argument('--noise_type', default=None, choices=[None, 'gaussian', 'poisson_gaussian'],
                   help='Override training.noise_type. Default: keep yaml value '
                        '(poisson_gaussian for the 3mean configs).')
    p.add_argument('--noise_seed', type=int, default=None,
                   help='Override training.noise_seed for reproducible noise.')
    p.add_argument('--noise_tau', type=float, default=None,
                   help='Override training.noise_tau (poisson_gaussian only).')
    p.add_argument('--noise_readout_snr', type=float, default=None,
                   help='Override training.noise_readout_snr (poisson_gaussian only).')
    p.add_argument('--noise_sigma', type=float, default=None,
                   help='Override training.noise_sigma (gaussian only).')
    p.add_argument('--log_every', type=int, default=400,
                   help='Override training.log_every. Default 400 = 10 sample points '
...
def main():
    ...
    print(f'[denoise-groups] device={device}')

    cfg_dir = Path(args.config_dir)
    if not cfg_dir.exists():
        fallback = Path('configs/experiments/image_fitting_3mean')
        if fallback.exists():
            print(f'[denoise-groups] {cfg_dir} not found; using fallback {fallback}')
            cfg_dir = fallback
        else:
    ...
    task_mod = get_task('image_denoising')
    ...
    for g in args.groups:
        for method in args.methods:
            ...
            cfg['task'] = 'image_denoising'
            t = cfg.setdefault('training', {})
            if args.noise_type is not None:        t['noise_type']        = args.noise_type
            if args.noise_seed is not None:        t['noise_seed']        = int(args.noise_seed)
            if args.noise_tau is not None:         t['noise_tau']         = float(args.noise_tau)
            if args.noise_readout_snr is not None: t['noise_readout_snr'] = float(args.noise_readout_snr)
            if args.noise_sigma is not None:       t['noise_sigma']       = float(args.noise_sigma)
            if args.log_every is not None:         t['log_every']         = int(args.log_every)
            ...
            for coords, pixels, meta in group_images[g]:
                ...
                if args.skip_existing and pt_path.exists():
                    ...
                    per_image.append({
                        ...
                        'noise_info':       loaded.get('noise_info'),
                        'noisy_input_psnr': loaded.get('noisy_input_psnr'),
                    })
                    continue
                ...
                try:
                    ...
                    per_image.append({
                        ...
                        'noise_info':       result.get('noise_info'),
                        'noisy_input_psnr': result.get('noisy_input_psnr'),
                    })
                    gain = result['best_psnr'] - result.get('noisy_input_psnr', 0.0)
                    print(f'    → best_PSNR={result["best_psnr"]:.2f}  '
                          f'best_SSIM={result["best_ssim"]:.4f}  '
                          f'noisy_PSNR={result.get("noisy_input_psnr", float("nan")):.2f}  '
                          f'gain={gain:+.2f}dB  ({dt:.1f}s)')
    ...
    for g in args.groups:
        for method in args.methods:
            ...
            row = {
                ...
                'avg_noisy_psnr':  _mean([d.get('noisy_input_psnr') for d in data]),
                'per_image': data,
            }
            agg_rows.append(row)
            def _fmt(v, nd=3):
                return 'n/a' if v is None else f'{v:.{nd}f}'
            gain = (row['avg_best_psnr'] - row['avg_noisy_psnr']
                    if row['avg_best_psnr'] is not None and row['avg_noisy_psnr'] is not None
                    else None)
            print(f"{g:8s}  {method:8s}  "
                  ...
                  f"gain={'n/a' if gain is None else f'{gain:+.2f}dB'}  "
                  f"(n={row['n_images']})")
    ...
    # Save Markdown table — pull the actually-used noise info
    used_ni = None
    for r in agg_rows:
        for d in r['per_image']:
            if d.get('noise_info') is not None:
                used_ni = d['noise_info']; break
        if used_ni is not None: break
    if used_ni is None:
        ni_label = '?'
    elif used_ni.get('noise_type') == 'poisson_gaussian':
        ni_label = (f"poisson_gaussian "
                    f"(tau={used_ni.get('tau')}, readout_snr={used_ni.get('readout_snr')})")
    elif used_ni.get('noise_type') == 'gaussian':
        ni_label = f"gaussian (sigma={used_ni.get('sigma')})"
    else:
        ni_label = str(used_ni)

    md_lines = [
        f'# Image Denoising ({ni_label}) on High / Mid / Low 5-image groups',
        '',
    ...
    def _md(v, nd=2):
        return 'n/a' if v is None else f'{v:.{nd}f}'
    for r in agg_rows:
        gain = (r['avg_best_psnr'] - r['avg_noisy_psnr']
                if r['avg_best_psnr'] is not None and r['avg_noisy_psnr'] is not None
                else None)
        md_lines.append(
            f"| {r['group']} | {r['method']} | {r['n_images']} | "
            f"{_md(r['avg_best_psnr'])} | {_md(r['avg_best_ssim'], 4)} | "
            f"{_md(r['avg_final_psnr'])} | {_md(r['avg_final_ssim'], 4)} | "
            f"{_md(r['avg_noisy_psnr'])} | "
            f"{'n/a' if gain is None else f'{gain:+.2f}'} |"
        )
