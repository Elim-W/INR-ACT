...
class SL2A(nn.Module):
    def __init__(self, in_features, hidden_features, hidden_layers, out_features, degree=512, outermost_linear=True, nonlinearity='relu', rank=32, init_method='xavier_uniform', linear_init_type='kaiming_uniform'):
        super(SL2A, self).__init__()
        self.nonlin = ChebyLayer
        self.net = nn.ModuleList()
        self.nonlinearity = nonlinearity
        self.rank = rank
        self.net.append(self.nonlin(in_features, hidden_features, deg=degree, init_method=init_method))
