"""
Multi-image hyperparameter search on image_inpainting.

Each trial fits the SAME (lr, omega, ...) on K images at the same
sampling_ratio, then aggregates the K PSNRs into one Optuna score.

Defaults:
    images: 0214 (low-mid) + 0221 (high) + 0527 (high-mid) from data/validation
    downscale=4   (each image's W and H divided by 4 with BICUBIC)
    sampling_ratio=0.2  (drop 80% pixels, keep 20%)
    mask_seed=0   (FIXED across all trials AND all images)
    agg=mean
...
def build_trial_cfg(method, params, iters, batch_size, sampling_ratio, mask_seed):
    ...
    train_cfg = {'num_epochs': iters, 'batch_size': batch_size,
                 ...
                 'sampling_ratio': sampling_ratio, 'mask_seed': mask_seed}
    ...
    return {'task': 'image_inpainting', 'method': method,
...
def run_single_image(method, params, coords, pixels, meta, device,
    ...
    return image_inpainting.run(model=model, coords=coords, pixels=pixels,
...
def _full_config_and_params(method, params, iters, batch_size, sampling_ratio, mask_seed):
    cfg = build_trial_cfg(method, params, iters, batch_size, sampling_ratio, mask_seed)
    mcfg = cfg['model']
    standard = ('in_features', 'hidden_features', 'hidden_layers', 'out_features')
    model = get_INR(method=method,
...
def save_method_txt(method, best_trial, tag, image_metas, n_trials, iters,
                    batch_size, sampling_ratio, mask_seed, agg, per_image_psnrs):
...
def _reset_trial_log(log_path, method, param_keys, tag, image_metas, iters, agg, sampling_ratio, mask_seed):
    with open(log_path, 'w') as f:
        f.write(f'# task: image_inpainting (multi)\n# method: {method}\n# tag: {tag}\n')
        f.write(f'# images: {[m["name"] for m in image_metas]}\n')
        f.write(f'# sampling_ratio: {sampling_ratio}\n# mask_seed: {mask_seed}\n# agg: {agg}\n')
...
def update_method_yaml(method, params):
    yaml_path = os.path.join(_ROOT, 'configs', 'experiments', 'image_inpainting', f'{method}.yaml')
...
def parse_args():
    ...
    p.add_argument('--downscale', type=int, default=4,
                   help='Divide W,H by this factor (BICUBIC); default 4')
    p.add_argument('--methods', nargs='+', default=None)
    p.add_argument('--n_trials', type=int, default=20)
    p.add_argument('--iters', type=int, default=4000)
    p.add_argument('--batch_size', type=int, default=-1)
    p.add_argument('--sampling_ratio', type=float, default=0.2,
                   help='Fraction of pixels kept for training (default 0.2 = drop 80%)')
    p.add_argument('--mask_seed', type=int, default=0,
                   help='Fixed across all trials and all images so PSNRs comparable')
...
def main():
    ...
    tag = f'multi_inp_{args.dataset}_{idx_str}_r{ratio_tag}_{args.agg}'

    print(f'[hparam_search_inpaint_multi] device={device}  agg={args.agg}  '
          f'sampling_ratio={args.sampling_ratio}  mask_seed={args.mask_seed}  '
    ...
    for method in methods:
        ...
        _reset_trial_log(log_path, method, param_keys, tag, image_metas, args.iters,
                         args.agg, args.sampling_ratio, args.mask_seed)
